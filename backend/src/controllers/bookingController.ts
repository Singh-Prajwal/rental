...
export const createBooking = async (req: Request, res: Response) => {
  try {
    ...
    try {
      await sendEmail({
        to: 'admin@urbanstay.com', // Your admin email
      ...
      try {
        if (bookingToUpdate.user && 'email' in bookingToUpdate.user) {
            await sendEmail({
                ...
                text: `Hi ${(bookingToUpdate.user as any).name},\n\nYour booking for ${bookingToUpdate.propertyName} is confirmed!\n\nCheck-in: ${new Date(bookingToUpdate.checkInDate).toLocaleDateString()}\nYour secure access code is: ${accessCode}\n\nEnjoy your stay!`,
                html: `
                    <h2>Your Booking is Confirmed!</h2>
                    <p>Hi <strong>${(bookingToUpdate.user as any).name}</strong>,</p>
                    <p>Your booking for <strong>${bookingToUpdate.propertyName}</strong> is confirmed and we can't wait to host you.</p>
                    <h3>Trip Details:</h3>
                    <ul>
                        <li><strong>Check-in:</strong> ${new Date(bookingToUpdate.checkInDate).toLocaleDateString()}</li>
                        <li><strong>Check-out:</strong> ${new Date(bookingToUpdate.checkOutDate).toLocaleDateString()}</li>
                    </ul>
                    ...
                    <p>Enjoy your stay!<br/>The UrbanStay Team</p>
                `
            });
            console.log(`Confirmation email sent to ${(bookingToUpdate.user as any).email}.`);
        }
      } catch (emailError) {
